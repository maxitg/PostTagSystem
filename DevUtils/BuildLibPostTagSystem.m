...
PackageExport["BuildLibPostTagSystem"]

Options[BuildLibPostTagSystem] = {
  "RepositoryDirectory" :> $PostTagSystemRoot,
...
SetUsage @ "
BuildLibPostTagSystem[] builds the libPostTagSystem library from source, and returns an association of \
metadata on completion, or $Failed if the library could not be built.
...
* By default, the sources are obtained from the 'libPostTagSystem' subdirectory of the current repo, but this \
...
* Additional metadata is written to 'LibraryTargetDirectory' in a file called 'libPostTagSystemBuildInfo.json'.
...
BuildLibPostTagSystem::compfail = "Compilation of C++ code at `` failed.";
BuildLibPostTagSystem::badsourcedir = "Source directory `` did not exist.";

BuildLibPostTagSystem[OptionsPattern[]] := ModuleScope[
  (* options processing *)
  UnpackOptions[
  ...
  (* path processing *)
  buildDataPath = FileNameJoin[{libraryTargetDirectory, "libPostTagSystemBuildInfo.json"}];
  ...
  (* derive final paths *)
  libraryFileName = StringJoin["libPostTagSystem-", finalHash, ".", System`Dump`LibraryExtension[]];
  ...
  If[!StringQ[libraryPath],
    Message[BuildLibPostTagSystem::compfail, librarySourceDirectory];
...
$printPreBuildCallback = Function[Print["Building libPostTagSystem from sources in ", #LibrarySourceDirectory]];
