<|
  "WolframModelEvolutionObject" -> <|
    ...
    "tests" -> With[{pathGraph17 = Partition[Range[17], 2, 1]}, {
      ...
      }], {method, DeleteCases[$PostTagSystemMethods, Automatic]}],
  ...
  "WolframModelEvolutionObjectGraphics" -> <|
    "init" -> (
      ...
      Global`testUnevaluated[args___] := PostTagSystem`PackageScope`testUnevaluated[VerificationTest, args];
      Global`checkGraphics[args___] := PostTagSystem`PackageScope`checkGraphics[args];
      Global`graphicsQ[args___] := PostTagSystem`PackageScope`graphicsQ[args];
    ),
    "tests" -> {
      (* FinalStatePlot *)
